...
int main(){
	...
	u32 LOCAL_ADC_u32_RES_VALUE=0;
     u16 LOCAL_ADC_u16_DIGITAL_VALUE=0;
	DIO_voidInit();
    ADC_enum_ADC_INIT();
    LCD_enum_INIT();
    LCD_enum_GO_TO_XY(LCD_u8_LINE1,1);
	LCD_enum_Send_String("hello");
    	_delay_ms(1000);
    	    	LCD_enum_Clear();
    while(1){
//    	ADC_enum_GET_DIGITAL_VALUE_ASynch(ADC_u8_CHANNEL_0,&ADC_void_ADC_Call_Back);
    	ADC_enum_GET_DIGITAL_VALUE_Synch_Non_Blocking(ADC_u8_CHANNEL_0,&LOCAL_ADC_u16_DIGITAL_VALUE);
    LOCAL_ADC_u16_ANALOG_VALUE=(u16)((LOCAL_ADC_u16_DIGITAL_VALUE * 5000UL)/1024) ; /*range of mv to avoid floating numbers */
    LOCAL_ADC_u32_RES_VALUE = ((10000*1024UL)/LOCAL_ADC_u16_ANALOG_VALUE) - 10000;
	...
	LCD_enum_GO_TO_XY(LCD_u8_LINE2,1);
	LCD_enum_Send_char('r');
	LCD_enum_Send_char(':');
    LCD_enum_Send_Num(LOCAL_ADC_u32_RES_VALUE);
    	_delay_ms(500);
    	LCD_enum_Clear();
